    ...
    def forward(self, root_batch):
        ...
        for t in range(max_depth):
            ...
            for node_x,node_y in prop_list:
                ...
                # 计算需要填充或截断的长度
                current_length = len(h_nei)
                if current_length > MAX_NB:
                    # 如果当前长度超过最大长度，进行截断操作
                    h_nei = h_nei[:MAX_NB]  # 切片保留前 MAX_NB 个元素
                elif current_length < MAX_NB:
                    # 如果当前长度不足最大长度，进行填充操作
                    pad_len = MAX_NB - current_length
                    padding_list = [padding] * pad_len
                    h_nei.extend(padding_list)  # 使用填充列表扩展至最大长度
                
                cur_h_nei.extend(h_nei)

            cur_x = create_var(torch.LongTensor(cur_x))
            cur_x = self.embedding(cur_x)
            # print(len(cur_h_nei),len(cur_h_nei[0]))
            # print(len(cur_x))
            # try:
