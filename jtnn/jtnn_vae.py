...
class JTNNVAE(nn.Module):
    ...
    def forward(self, mol_batch, is_pre = False, alpha = 0, beta=0, ):
        if is_pre:
            batch_size = len(mol_batch)
            random.shuffle(mol_batch)

            mol_batch_tree = mol_batch

            tree_mess, tree_vec, mol_vec= self.encode(mol_batch_tree)
            ...
            word_loss, topo_loss, word_acc, topo_acc = self.decoder(mol_batch_tree, tree_vec)
            assm_loss, assm_acc = self.assm(mol_batch_tree, mol_vec, tree_mess)
            if self.use_stereo:
                stereo_loss, stereo_acc = self.stereo(mol_batch_tree, mol_vec)
            else:
            ...
            mol_batch_tree = list(map(itemgetter(0), mol_batch))
            mol_batch_label = list(map(itemgetter(1), mol_batch))
            cal_labels = torch.tensor(mol_batch_label, requires_grad=False, device='cuda:0')
            ...
            all_vec = torch.cat([tree_vec, mol_vec], dim=1)
            property_loss = self.pro(all_vec, mol_batch_label)
            
            loss = word_loss + topo_loss + assm_loss + 2 * stereo_loss + beta * kl_loss + alpha * property_loss

            return loss, kl_loss.item(), word_acc, topo_acc, assm_acc, stereo_acc, alpha * property_loss

    
    def pro(self, mol_vec, mol_label):
        cal_labels = torch.tensor(mol_label, requires_grad=False)
        certain_values = cal_labels.unique()
        certain_values = certain_values[certain_values!=-1.0]
        code_list = []
        
        if certain_values.shape[0] == 0:  # no labeled data
            return torch.tensor(0.0, dtype=torch.float32)
     
        # at least one kind of property
        same_property_dist = 0
        same_cate_num = 0
        for certain_value in certain_values:
            certain_index = (cal_labels == certain_value)
            same_property_vectors = mol_vec[certain_index, :]
            code_list.append(same_property_vectors)
            same_cate_num += 1
            if same_property_vectors.shape[0] == 1:  # only one data in a set with the same property 
                continue
            dist_mean = F.pdist(same_property_vectors, p=2).mean()
            same_property_dist += dist_mean

        same_property_dist_mean = same_property_dist / same_cate_num 

        if certain_values.shape[0] == 1:  # only one kind of property
            return same_property_dist_mean
        
        # at least two kind of property
        different_property_combination = list(itertools.combinations(code_list, 2))
        different_property_dist = 0
        for i, property_comb in enumerate(different_property_combination):
            dist_mean = torch.mean(torch.cdist(property_comb[0], property_comb[1], p=2))
            different_property_dist += dist_mean
        
        different_property_dist_mean = different_property_dist / len(different_property_combination)
        
        property_loss = same_property_dist_mean - different_property_dist_mean
        
        return property_loss
    

    def pro_triplet(self, mol_vec, mol_label, margin=1.0, num_triplets=40):
        labels = torch.tensor(mol_label) if not isinstance(mol_label, torch.Tensor) else mol_label
        device = mol_vec.device

        valid_mask = labels != -1.0
        anchor_vec = mol_vec[valid_mask]
        anchor_labels = labels[valid_mask]

        pairwise_dist = torch.cdist(anchor_vec, anchor_vec, p=2)

        matches = (anchor_labels.unsqueeze(0) == anchor_labels.unsqueeze(1)).float()
        diffs = 1 - matches
        ...
        valid_triplets = (matches.unsqueeze(2) * diffs.unsqueeze(1)).bool()
        loss = loss[valid_triplets]
        
        return loss.mean() if len(loss) > 0 else torch.tensor(0.0, device=device)
    ...
    def decode(self, tree_vec, mol_vec, prob_decode):
        ...
        scores = nn.CosineSimilarity()(stereo_vecs, mol_vec) # [2]
        _,max_id = scores.max(dim=0)
        # print("jtnn_vae.decoder: _, max_id", _, max_id)
